...
//Ecouter la modification de Password Verfy
form.passv.addEventListener('change', function () {
    validpassv(this);
  });
...
form.addEventListener('submit', function (e) {
  ...
  if (validname(form.user) && validpass(form.pass) && validpassv(form.passv)) {
   form.submit();
...
const validpassv = function () {
    let msg = document.getElementById('pas');
    let valid_pass=document.getElementById('pass').value;
    let valid_passv=document.getElementById('passv').value;
    if (valid_passv==valid_pass) {
        msg.innerHTML=``;
        msg.innerHTML = `<p  style="color:#008000;">Votre Password Valide</p>`;
        return true;
       }
       else 
       {
        msg.innerHTML=``;
       msg.innerHTML = `<p  style="color:red;">Il faut entrer le même password !!</p>`;
       return false;
      }
